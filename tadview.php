...
class	systeminfo {
	...
	function	hook_start() {
		...
		if ($systeminfo->apache_child_terminate) {
...
class	array_bytestream extends bytestream {
	...
	function	array_bytestream(&$uh_array) {
		$this->buffer =& $uh_array;
	...
	function	&clone_stream() {
		...
		$newstream->end = $this->end;
		$newstream->pos = $this->pos;
		$newstream->parent_infostr = $this->infostr;
		return $newstream;
	}
	function	&subst_stream($size) {
		$newstream =& new array_bytestream();
	...
	function	getinfostr() {
		return sprintf("stream offset(%08x) in array top(%08x) end(%08x) from %s", $this->pos, $this->path, $this->top, $this->end, $this->parent_infostr);
	}
}

class	memory_bytestream extends array_bytestream {
	function	memory_bytestream(&$stream, $size) {
		$array = array();
		while ($size > 0) {
			$array[] = $stream->uh();
			$size -= 2;
...
#			$this->uh($stream->subst_stream($len));
			$this->uh(new memory_bytestream($stream, $len));
		}
	}
	...
	function	&create($key, $type, &$stream) {
		...
		switch (($type << 8)|($subtype >> 8)) {
			...
			case	0xffa202:
				...
				switch ($val & 0xc000) {
					case	0:
						$this->add("charsize", $val);
						break;
					case	0x4000:
						$this->add("charsize", $this->todpc($val, $this->scope));
						break;
					case	0x8000:
						$this->add("charsize", $this->todpi($val, $this->scope));
						break;
				}
				return $this->scope->factoryfinder->read("000d0000", 7, $stream);
			case	0xffa203:
...
class	tadfactory_vobjparts extends tadfactory_parts {
	...
	function	&create($key, $type, &$stream) {
		...
		$stream->uh();	# height
		$stream->uh();	# fontsize (if opened)
		$newparts->frcolor = $stream->uw();	# frame color
		$stream->uw();	# text color
	...
	function	draw(&$genv, &$left, &$top, &$right, &$bottom, $draw = 1) {
		...
		if (($predraw)) {
			...
			while ($this->partslist->remain() > 0) {
				$target =& $this->partslist->get();
				$left = $top = 0;
#				$target->draw($genv_pre, $left, $top, $right, $bottom, $predraw);
...
class	genv_gd2 extends genv {
	...
	var	$first = 1;
	function	genv_gd2($dpi_h, $dpi_v) {
		parent::genv($dpi_h, $dpi_v);
...
class	genv_png extends genv_gd2 {
	function	genv_png($dpi_h, $dpi_v) {
		global	$global;
		
		parent::genv_gd2($dpi_h, $dpi_v);
...
class	genv_jpeg extends genv_gd2 {
	function	genv_jpeg($dpi_h, $dpi_v) {
		global	$global;
	...
	function	flushpage() {
		...
		for ($y=$this->pageinfo[7]-1; $y>=0; $y--)
			for ($x=0; $x<$this->pageinfo[6]; $x++) {
				...
				pdf_clip($this->pid);
				pdf_place_image($this->pid, $this->inpage, -$pitch_h * $x, -$pitch_v * $y + $this->pageinfo[5], 1);
...
class	genv_pdf6 extends genv_pdf {
	...
	var	$fontidlist;
	var	$fontkey = "";
	function	genv_pdf6($dpi_h, $dpi_v) {
		global	$global;
		
		parent::genv_pdf($dpi_h, $dpi_v);
...
class	genv_pdfzf extends genv {
	...
	var	$pagecount = 1;
	var	$fontlist;
	function	genv_pdfzf($dpi_h, $dpi_v) {
		global	$global;
...
$basescope->factoryfinder->readall(new file_bytestream($fn));

$basescope->partslist->rewind();
$target =& $basescope->partslist->get();

$dpih = $target->pageattrlist->get("dpih");
$dpiv = $target->pageattrlist->get("dpiv");
switch ($global->req->output) {
	case	-2:
		$genv =& new genv_log($dpih, $dpiv);
		break;
	default:
	case	1:
		if ($global->req->jpeg == 0) {
			$genv =& new genv_png($dpih, $dpiv);
			break;
		}
	case	2:
		$genv =& new genv_jpeg($dpih, $dpiv);
		break;
	case	3:
#		if ($systeminfo->zfpath !== null)
#			$genv =& new genv_pdfzf($dpih, $dpiv);
#		else
			$genv =& new genv_pdf($dpih, $dpiv);
		break;
	case	4:
		if ($systeminfo->zfpath !== null)
			$genv =& new genv_pdfzf($dpih, $dpiv);
		else
			$genv =& new genv_pdf6($dpih, $dpiv);
		break;
	case	5:
		$genv =& new genv_svg($dpih, $dpiv);
		break;
}
$basescope->draw($genv);
